...
class FlorenceEncoderAdapter(nn.Module):
    """Adapter to use Florence-style local model as a frozen feature extractor with prompt support.
    ...
    def __init__(self, model_path: str = './model/florence-2-base', pretrained: bool = True, feature_dim: int = 256, freeze_stages: int = 4, use_lora: bool = True, use_prompt: bool = True):
        super().__init__()
        ...
        vision_config = {
            ...
            'embed_dims': [128, 256, 512, 1024],  # Updated to match pre-trained checkpoint dimensions
            'num_heads': [8, 16, 32, 64],  # Updated to match new embed_dims (128/16=8, 256/16=16, etc.)
            'num_groups': [8, 16, 32, 64],  # Updated to match new embed_dims
            'window_size': 12,
        ...
        # Create 4 groups of learnable prompt tokens only if use_prompt is True
        if self.use_prompt:
        ...
        # Add LoRA to the vision encoder's later layers if enabled
        if self.use_lora:
    ...
    def add_lora_to_encoder(self):
        # Add LoRA to the later layers of the vision encoder
        # DaViT has 4 stages with different depths: [1, 1, 9, 1] blocks
        # We'll add LoRA to the last 30% of the blocks
        ...
        # Apply LoRA to the selected blocks
    ...
    def _freeze(self):
        ...
        # Keep the normalization layer trainable
    ...
    def _extract_feature_map(self, x: torch.Tensor, task_id: str = None) -> torch.Tensor:
        ...
        # Use detection path for all tasks if prompt is disabled
        if is_detection or not self.use_prompt:
            # --- Detection task or no prompt: No prompt, direct visual feature extraction ---
            ...
            # --- Non-detection tasks with prompt: Use prompt tokens ---
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('✅ FlorenceEncoderAdapter initialized successfully!')
        ...
        print('\nThe adapter is ready for use in the training pipeline (train_florence.py).')
        print('Running florence_adapter.py directly only verifies initialization, not forward pass.')
