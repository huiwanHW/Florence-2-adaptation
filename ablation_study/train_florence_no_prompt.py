...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('train_florence_no_prompt.log'),
...
epoch_logger.addHandler(logging.FileHandler('output_train_florence_no_prompt.log'))
...
# Override model_factory to use no prompt adapter
# Replace the import in model_factory
import model_factory
model_factory.FlorenceEncoderAdapter = None

# Import the no prompt adapter
from ablation_study.florence_adapter_no_prompt import FlorenceEncoderAdapter
...
def main():
    ...
    logger.info("=== ABLATION: w/o Prompt (soft prompt disabled) ===")
    ...
    for epoch in range(args.epochs):
        ...
        if avg_epoch_loss < best_train_loss:
            ...
            # Save model with filename best_model_no_prompt.pth
            torch.save(model.state_dict(), 'best_model_no_prompt.pth')
            logger.info(f'\nNew best model saved (train loss = {best_train_loss:.6f}) as best_model_no_prompt.pth')
            epoch_logger.info(f'\nNew best model saved (train loss = {best_train_loss:.6f}) as best_model_no_prompt.pth')
        ...
        if new_score > best_val:
            best_val = new_score
            torch.save(model.state_dict(), 'best_florence_no_prompt.pth')
            print(f'New best overall model saved (综合评分 = {best_val:.6f}) as best_florence_no_prompt.pth')
            # 记录各任务的最佳表现
            try:
                if not grouped_eval['segmentation'].empty and 'Dice' in grouped_eval['segmentation'].columns:
