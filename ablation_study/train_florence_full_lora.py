...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('train_florence_full_lora.log'),
...
epoch_logger.addHandler(logging.FileHandler('output_train_florence_full_lora.log'))
...
# Override model_factory to use full lora adapter
# Replace the import in model_factory
import model_factory
model_factory.FlorenceEncoderAdapter = None

# Import the full lora adapter
from ablation_study.florence_adapter_full_lora import FlorenceEncoderAdapter
...
def main():
    ...
    logger.info("=== ABLATION: Full LoRA (LoRA applied to ALL transformer layers) ===")
    ...
    for epoch in range(args.epochs):
        ...
        if avg_epoch_loss < best_train_loss:
            ...
            # Save model with filename including full lora
            torch.save(model.state_dict(), 'best_model_full_lora.pth')
            logger.info(f'\nNew best model saved (train loss = {best_train_loss:.6f}) as best_model_full_lora.pth')
            epoch_logger.info(f'\nNew best model saved (train loss = {best_train_loss:.6f}) as best_model_full_lora.pth')
        ...
        try:
            ...
            # Print per-group metrics with required metric names
            # Segmentation: Dice (DSC), Hausdorff (HD95)
            if not grouped_eval['segmentation'].empty:
                logger.info('\n[Validation] Segmentation metrics:')
                # Rename Hausdorff column to HD95 for consistency
                seg_df = grouped_eval['segmentation'].copy()
                if 'Hausdorff (px)' in seg_df.columns:
                    seg_df = seg_df.rename(columns={'Hausdorff (px)': 'HD95'})
                logger.info(seg_df.to_string(index=False))
            ...
            if not grouped_eval['classification'].empty:
                ...
                cls_df = grouped_eval['classification'].copy()
                logger.info(cls_df.to_string(index=False))

            # Detection: mIoU
            if not grouped_eval['detection'].empty:
                logger.info('\n[Validation] Detection metrics:')
                # Rename IoU column to mIoU for consistency
                det_df = grouped_eval['detection'].copy()
                if 'IoU' in det_df.columns:
                    det_df = det_df.rename(columns={'IoU': 'mIoU'})
                logger.info(det_df.to_string(index=False))

            # Regression: MRE
            if not grouped_eval['regression'].empty:
                logger.info('\n[Validation] Regression metrics:')
                # Ensure we have MRE metric
                reg_df = grouped_eval['regression'].copy()
        ...
        if new_score > best_val:
            best_val = new_score
            torch.save(model.state_dict(), 'best_florence_full_lora.pth')
            logger.info(f'New best overall model saved (综合评分 = {best_val:.6f}) as best_florence_full_lora.pth')
            # 记录各任务的最佳表现
            try:
                if not grouped_eval['segmentation'].empty and 'Dice' in grouped_eval['segmentation'].columns:
                    ...
                    if avg_dice > best_metrics['segmentation']['Dice']:
                        ...
                        logger.info(f'  当前最佳分割 Dice: {avg_dice:.6f}')
                ...
                if not grouped_eval['classification'].empty and 'AUC' in grouped_eval['classification'].columns:
                    ...
                    if avg_auc > best_metrics['classification']['AUC']:
                        ...
                        logger.info(f'  当前最佳分类 AUC: {avg_auc:.6f}')
                ...
                if not grouped_eval['detection'].empty and 'IoU' in grouped_eval['detection'].columns:
                    ...
                    if avg_iou > best_metrics['detection']['IoU']:
                        ...
                        logger.info(f'  当前最佳检测 IoU: {avg_iou:.6f}')
                ...
                if not grouped_eval['regression'].empty and 'MRE (pixels)' in grouped_eval['regression'].columns:
                    ...
                    if avg_mre < best_metrics['regression']['MRE']:
                        ...
                        logger.info(f'  当前最佳回归 MRE: {avg_mre:.6f}')
            except Exception as e:
