...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('train_florence.log'),
...
# Override model_factory to use lora adapter
import sys
...
def setup_logger(freeze_percent):
    """Setup logger for specific freeze percentage"""
    import logging
    logger = logging.getLogger(f"train_{freeze_percent}%")
    ...
    # Create file handler
    log_file = f"{freeze_percent}%_train_florence.log"
...
def main():
    ...
    # Initialize model with fixed freeze percentage
    logger = setup_logger(freeze_percent)
    logger.info(f"Starting training with parameters: {args}")
    logger.info(f"Using device: {device}")
    logger.info(f"Dataset size: {dataset_size}, Train size: {train_size}")
    logger.info("Data loaders initialized")

    # Modified MultiTaskModelFactory to accept freeze_percent parameter
    from model_factory import MultiTaskModelFactory as OriginalMultiTaskModelFactory
    
    class ModifiedMultiTaskModelFactory(OriginalMultiTaskModelFactory):
        def __init__(self, encoder_name, encoder_weights, task_configs, freeze_stages=4, freeze_percent=None):
            self.freeze_percent = freeze_percent
            super().__init__(encoder_name, encoder_weights, task_configs, freeze_stages)
        
        def _create_encoder(self):
            """Override to pass freeze_percent to FlorenceEncoderAdapter"""
            from florence_adapter import FlorenceEncoderAdapter
            ...
            self.encoder = FlorenceEncoderAdapter(
                ...
                freeze_percent=self.freeze_percent
            )
    ...
    def setup_optimizers(model):
        ...
        logger.info(f"设置学习率：分割任务 = {seg_lr}, 其他任务 = {other_lr}")
        ...
        return seg_optimizer, other_optimizer, seg_scheduler, other_scheduler, seg_original_lrs, other_original_lrs
    ...
    for epoch in range(total_epochs):
        # 设置logger
        logger = setup_logger(freeze_percent)
        logger.info(f"=== 开始训练epoch: {epoch+1}/{total_epochs}, 冻结比例: {freeze_percent}% ===")
        logger.info(f"Starting training with parameters: {args}")
        logger.info(f"Using device: {device}")
        logger.info(f"Dataset size: {dataset_size}, Train size: {train_size}")
        
        # 创建新模型
        model = ModifiedMultiTaskModelFactory(
        ...
        logger.info(f"Model initialized with freeze percentage: {freeze_percent}%")
        ...
        # 设置优化器
        seg_optimizer, other_optimizer, seg_scheduler, other_scheduler, seg_original_lrs, other_original_lrs = setup_optimizers(model)
        ...
        logger.info(f"Epoch {epoch+1}/{total_epochs}: 分割任务权重因子 = {seg_weight_factor:.4f}")
        ...
        for batch in tqdm(train_loader, desc=f'Epoch {epoch+1}/{total_epochs} [Train]', disable=False, leave=True):
        ...
        logger.info(f'[Train] Epoch {epoch+1}/{total_epochs}, Average Loss: {avg_epoch_loss:.6f}')
        ...
        if avg_epoch_loss < best_metrics['best_train_loss']:
            best_metrics['best_train_loss'] = avg_epoch_loss
            # Save model with filename including freeze percentage
            torch.save(model.state_dict(), f'{freeze_percent}%_best_model.pth')
            logger.info(f'\nNew best model saved (train loss = {best_metrics["best_train_loss"]:.6f}) as {freeze_percent}%_best_model.pth')
    
    # 训练完成后，输出最佳表现
    logger = setup_logger(freeze_percent)
    logger.info(f"=== 冻结比例: {freeze_percent}% 训练完成，最佳表现 ===")
    logger.info(f"最佳训练损失: {best_metrics['best_train_loss']:.6f}")
    logger.info(f"最佳模型已保存为: {freeze_percent}%_best_model.pth")
