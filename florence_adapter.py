...
class FlorenceEncoderAdapter(nn.Module):
    ...
    def __init__(self, model_path: str = './model/florence-2-base', pretrained: bool = True, feature_dim: int = 256, freeze_stages: int = 4, freeze_percent: int = 70):
        super().__init__()
        self.model_path = model_path
        self.feature_dim = feature_dim
        self.pretrained = pretrained
        self.freeze_stages = freeze_stages  # 0: no freeze, 1-4: freeze first N stages
        self.freeze_percent = freeze_percent  # 冻结百分比
    ...
    def _freeze(self):
        # Freeze specified percentage of the vision encoder's layers
        # DaViT has 4 stages with different depths: [1, 1, 9, 1] blocks
        ...
        # Calculate number of blocks to freeze (first freeze_percent%)
        num_freeze_blocks = int(total_blocks * (self.freeze_percent / 100))
        print(f"Number of blocks to freeze: {num_freeze_blocks} (based on {self.freeze_percent}% freeze)")
